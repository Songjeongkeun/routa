...
export async function signup({ loginId: rawLoginId, email: rawEmail, password, nickname: rawNickname }) {
  const loginId = rawLoginId?.trim()
  const email = rawEmail?.trim().toLowerCase()
  const nickname = rawNickname?.trim()

  if (!loginId || !email || !password || !nickname) {
    throw serviceError("아이디, 이메일, 비밀번호, 닉네임은 필수입니다.", 400)
  }
  if (loginId.length < 4 || loginId.length > 30) {
    throw serviceError("아이디는 4자 이상 30자 이하로 입력해주세요.", 400)
  }
  if (password.length < 8) {
    throw serviceError("비밀번호는 8자 이상으로 입력해주세요.", 400)
  }
  if (await authRepository.findByLoginId(loginId)) {
    throw serviceError("이미 사용 중인 아이디입니다.", 409)
  }
  ...
  try {
    ...
    const user = await authRepository.createUser({ loginId, email, passwordHash, nickname })
...
export async function login({ loginId: rawLoginId, password }) {
  const loginId = rawLoginId?.trim()
  if (!loginId || !password) {
  ...
  if (!user || !matches) {
    throw serviceError("아이디 또는 비밀번호가 올바르지 않습니다.", 401)
  }
